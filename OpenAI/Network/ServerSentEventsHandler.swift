...
class ServerSentEventsHandler<Success>: NSObject, URLSessionDataDelegate where Success: Decodable {
    
    var onEventReceived: ((Result<Success, Error>) -> Void)?
    var onComplete: (() -> Void)?
    ...
    func processEvent(_ eventData: Data) {
        do {
            let res = try JSONDecoder().decode(Success.self, from: eventData)
