            ...
            for choice in response.choices { // This only returns requests for the first choice - TODO: Add handling of multiple choices
                ...
                for tool_call in tool_calls {
                    if case .function(let function) = tools?.first(where: { $0.name == tool_call.function.name }) {
                        // verify arguments
                        guard let args = tool_call.function.arguments.data(using: .utf8).flatMap({ try? JSONSerialization.jsonObject(with: $0, options: []) as? [String:Any] }) else { throw ChatCompletionError.failedToDecodeFunctionArguments }
